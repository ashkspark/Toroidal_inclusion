% ------------------------------------------------------------------- 
% Ashkan Golgoon, Num. Methods for PDEs, Fall 2016, Final Project
% Instructor: Prof.Liu
% -------------------------------------------------------------------
...
A13 = D1y;
%------------------------------
A21 = diag(2.*sin(X(:))./Y(:).^2./sx1)...
    -diag(1./Y(:).*sin(2.*X(:))./sx1.^2)...
...
A0 = [A11,A12,A13; A21,A22,A23; A31,A32,A33];  
...
F0 = zeros(3*m*n,1);
F0(2*m*n+1:3*m*n,1) = ...
    +3./2.*Y(:).*sx1.*(1./2.*(1-erf((Y(:)-Ri)./sigma))).*omega;
...
lqu = 2./Bo./delta1.^2.*D0-3./delta1.^3.*D1x-1./delta1.^2.*D2xy;  
lqw = 2./Bo./delta1.*D1x-3./delta1.*D1y-D2y-2./delta1.^2.*D2x;  
lqp = 1./delta1^2.*D1x;
...
%-----------------------------------------------
title(['$\delta\Omega=$',num2str(omega),'$\,,\frac{R_i}{R_o}=$'...
    ,num2str(1/pc),'$\,,B=$',num2str(Bo),'$\,,\delta b=$',num2str(db)...
    ,'$\,.$'],'Interpreter','Latex');
%-------------------------------------------------------------------------