...
%}
%--------------------------------------------------------------------------
n = 21; %number of grids in the circumferential direction.
m = 81; %number of grids in the radial direction
s = 1;
%--------------------------------------------------------------------------
...
A0 = [A11,A12,A13; A21,A22,A23; A31,A32,A33];  %creating the 3mn*3mn matrix


cn=[+2.*cos(X(:))./sx1.^2;
    -2./Y(:).*sin(X(:))./sx1.^2;
    Y(:)];
A = [[A0,cn];zeros(1,3*m*n+1)];

F0 = zeros(3*m*n,1);
F0(2*m*n+1:3*m*n,1) = +3./2.*Y(:).*sx1.*(1./2.*(1-erf((Y(:)-Ri)./sigma))).*omega;
...
if strcmp(method,'reduce')    
    fs = [ones(n,1), sin(x*(1:s)), cos(x*(1:s))];
    R0 = kron(speye(3),kron(Iy,fs));
    R1 = kron(speye(3),kron(Iy,fs'*diag(S)));
...
xlabel('$X$','Interpreter','latex');
ylabel('$Y$','Interpreter','latex');
zlabel('$w(X,Y)$','Interpreter','latex');
%--------------------------------------------
subplot(1,3,3);
...
xlabel('$X$','Interpreter','latex');
ylabel('$Y$','Interpreter','latex');
zlabel('$\delta p(X,Y)$','Interpreter','latex');
%-----------------------------------------------
title(['$\delta\Omega=$',num2str(omega),'$\,,\frac{R_i}{R_o}=$',num2str(1/pc),...
    '$\,,B=$',num2str(Bo),'$\,,\delta b=$',num2str(db),'$\,.$'],'Interpreter','Latex');


%gr11r=legend(p1,['$nx=$',num2str(nx),'$\,,nt=$',num2str(nt),...
    %'$\,,r=$',num2str(r),'$\,.$'],'Location','southeast');
%set(gr11r,'Interpreter','Latex');
